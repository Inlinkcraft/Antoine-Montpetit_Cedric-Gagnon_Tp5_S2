...
public class MessageChiffrerDechiffrerTest
{
	...
	{
		mCD1 = new MessageChiffrerDechiffrer(new VecteurDeCaracteres(),
				new ListeMatricesChiffrement(1, 15, 3, 28), FichierUtilitaires
						.lireDictionnaire(new File("dictionnaire.txt")));
		mCD2 = new MessageChiffrerDechiffrer(new VecteurDeCaracteres(),
				new ListeMatricesChiffrement(1, 16, 3, 28), FichierUtilitaires
						.lireDictionnaire(new File("dictionnaire.txt")));
		mCD3 = new MessageChiffrerDechiffrer(new VecteurDeCaracteres(),
				new ListeMatricesChiffrement(1, 17, 3, 28), FichierUtilitaires
						.lireDictionnaire(new File("dictionnaire.txt")));
	}
	...
	public void testGetMatriceCourante()
	{
		...
		assertTrue(mCD1.getMatriceCourante().length == 3
				&& mCD1.getMatriceCourante()[0].length == 3);
		assertTrue(mCD2.getMatriceCourante().length == 3
				&& mCD2.getMatriceCourante()[0].length == 3);
		assertTrue(mCD3.getMatriceCourante().length == 3
				&& mCD3.getMatriceCourante()[0].length == 3);
	}

	@Test
	public void testValiderMessageSelonDico()
	{
		assertTrue(mCD1.validerMessageSelonDico("IL EST LE MEILLEUR AU MONDE",
				0.1f));
		assertTrue(
				mCD2.validerMessageSelonDico("MAIS LUI LES TOUS AUSSI", 0.1f));
		assertTrue(mCD3.validerMessageSelonDico(
				"JE CROIT QUE CES PHRASE FONCTIONNE", 0.1f));
		assertFalse(mCD1.validerMessageSelonDico("IMA BREAK YOU", 0.1f));
		assertFalse(mCD2.validerMessageSelonDico(
				"ENGLIDH IS NOT ITS STRONG POINT", 0.1f));
		assertFalse(mCD3.validerMessageSelonDico(
				"WELL WELL A LEGAL PLACE FOR ENGLISH XD", 0.1f));
	}
	...
	public void testAjusterMessageBrute()
	{
		...
		assertEquals(mCD2.ajusterMessageBrute("Antoine", 10),
				"Antoine          ");
		assertEquals(mCD1.ajusterMessageBrute("Cedric", 12),
				"Cedric            ");
	}
	...
	public void testEncoder()
	{
		...
		/*
		 * 
		 * Cette méthode ne peut être testé de façons rigoureuse car nous
		 * n'avons pas acces a la liste de matrice candidate qui ce trouve a
		 * l'intérieur du messageChiffrerDéchiffrer
		 * 
		 * j'ai alors écrit ce code qui ne fait que comparer si la sortie etait
		 * suffisament différente
		 * 
		 */
